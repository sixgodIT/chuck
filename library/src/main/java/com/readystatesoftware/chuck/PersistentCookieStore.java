 ...
 * @date Created at 下午7:43 on 2017/6/8
 * @description
 */
public class PersistentCookieStore implements CookieStore {
    ...
    public void setCookieListener(CookieListener cookieListener) {
        mCookieListener = cookieListener;
    }

    @Override
    public void add(URI uri, HttpCookie cookie) {
        if (mCookieListener != null) {
            mCookieListener.addCookie(cookie);
        }
        ...
        if (!cookie.hasExpired()) {
            ...
            if (mCookies.containsKey(cookie.getDomain()))
                mCookies.get(cookie.getDomain()).remove(cookie.getName());
        }
    ...
    private void addKey(ArrayList<HttpCookie> ret, String key) {
        ConcurrentHashMap<String, HttpCookie> concurrentHashMap = mCookies.get(key);
        Set<Map.Entry<String, HttpCookie>> entrySet = concurrentHashMap.entrySet();
        Iterator<Map.Entry<String, HttpCookie>> iterator = entrySet.iterator();
        while (iterator.hasNext()) {
            Map.Entry<String, HttpCookie> entry = iterator.next();
            if (entry.getValue().hasExpired()) {
//                System.out.println("], key = [" + key + "]" + entry.getValue().getName());
    ...
    public boolean remove(URI uri, HttpCookie cookie) {
        ...
        if (mCookies.containsKey(cookie.getDomain()) && mCookies.get(cookie.getDomain()).containsKey(cookie.getName())) {
            mCookies.get(cookie.getDomain()).remove(cookie.getName());

            SharedPreferences.Editor prefsWriter = mCookiePrefs.edit();
            if (mCookiePrefs.contains(COOKIE_NAME_PREFIX + cookie.getName())) {
